...
class LicenseInvalidError extends Error {
    constructor(message) {
        ...
        message
            ? super(`license is invalid: ${message}`)
...
class DataAbnormalError extends Error {
    constructor(message) {
        message
            ? super(`data is abnormal: ${message}`)
            : super("data is abnormal");
    }
}
...
class VoerkaLicenseWatcher {
    ...
    async checkLicense() {
        ...
        if (valid) {
            this.rawLicenseData = querystring.parse(
            ...
            this.validBeginAt = $validBeginAt
                ? new Date(Number($validBeginAt) || $validBeginAt || 0)
                : new Date();
            this.validEndAt = $validEndAt
                ? new Date(Number($validEndAt) || $validEndAt || 0)
                : new Date("2099-12-31");
            if (
                Number.isNaN(this.validBeginAt.getTime()) ||
                Number.isNaN(this.validEndAt.getTime())
            ) {
                valid = this[CALLBACK](EVENTS.invalid);
            } else {
                // Si se especifica la hora efectiva, se sobrescribirá la fecha límite efectiva, por lo que se volverá a calcular.
                this.validDuration = $validDuration
                    ? ms($validDuration)
                    : this.validEndAt - this.validBeginAt;
                this.validEndAt = new Date(
                    this.validBeginAt.getTime() + this.validDuration
                );
                this.devices = $devices
                    ? $devices.split(",").filter((x) => x)
                    : [];
                this.deviceTypes = $deviceTypes
                    ? $deviceTypes.split(",").filter((x) => x)
                    : [];
                let { sn: deviceSn, type: deviceType } = this.device;
                if (this.devices.length && !this.devices.includes(deviceSn)) {
                    valid = this[CALLBACK](
                        EVENTS.invalid,
                        new DeviceNotInLicensedListError(deviceSn)
                    );
                } else if (
                    deviceType &&
                    ...
                    valid = this[CALLBACK](
                        EVENTS.invalid,
                        new DeviceTypeNotInLicensedListError(deviceType)
                    );
                }
                $deviceNumbers = Number($deviceNumbers);
                $accountNumbers = Number($accountNumbers);
                $useTimes = Number($useTimes);
                this.deviceNumbers = Number.isInteger($deviceNumbers)
                    ? $deviceNumbers
                    : 0;
                this.accountNumbers = Number.isInteger($accountNumbers)
                    ? $accountNumbers
                    : 0;
            ...
            this[CALLBACK](
                EVENTS.invalid,
                this.licenseInvalidError || new LicenseInvalidError()
            );
        }
    ...
    genCertificate() {
        ...
        | VERSION              |${paddingLeft(
            VERSION,
            22
        )}${VERSION}${paddingRight(VERSION, 22)}|
        +----------------------+----------------------+
        | RELEASED AT          |${paddingLeft(
            RELEASED_AT,
            22
        )}${RELEASED_AT}${paddingRight(RELEASED_AT, 22)}|
        +----------------------+----------------------+
        | VALID BEGIN AT       |${paddingLeft(
            VALID_BEGIN_AT,
            22
        )}${VALID_BEGIN_AT}${paddingRight(VALID_BEGIN_AT, 22)}|
        +----------------------+----------------------+
        | VALID END AT         |${paddingLeft(
            VALID_END_AT,
            22
        )}${VALID_END_AT}${paddingRight(VALID_END_AT, 22)}|
        +----------------------+----------------------+
        `;
    }
    async [CHK_DURA]() {
        try {
            if (!this.licensePassed)
                this[CALLBACK](
                    EVENTS.invalid,
                    this.licenseInvalidError || new LicenseInvalidError()
                );
                ...
                if (duration < 0) {
                    this[CALLBACK](
                        EVENTS.invalid,
                        new LicenseAheadofBeginDateError()
                    );
    ...
    async getDuration() {
        if (!this.validatedDuration)
            this.validatedDuration = (await this.getData()).validatedDuration;
    ...
    async stop() {
        ...
        this.checking &&
            (await new Promise((resolve) => this.on(EVENTS.checked, resolve)));
