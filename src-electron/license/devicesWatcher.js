/**
 * Para la certificación de equipos
 */
require("bytenode");
...
//Certificado predeterminado (para evitar que se elimine el certificado y que el observador informe un error. Si no desea el valor predeterminado 50 más adelante, puede asignar directamente una cadena aleatoria, siempre que no esté vacía).
...
// Obtener oyente
module.exports = function getWatcher({
    licenseFile,
    ...
    if (
        ...
        try {
            ...
            // Supervisar los cambios de certificado y reasignarlos al observador
            ...
            // Cuando ocurre una excepción de monitoreo, es decir, cuando el certificado expira, haga algo
            throw new Error(`create watcher error：${e.message}`);
        }
    } else {
        // Cuando se altera el archivo del monitor de certificados de Voerka, es decir, cuando el certificado no es válido, haga algo
